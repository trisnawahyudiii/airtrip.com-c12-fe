...
const FlightCard = ({ flight, handleSelectFlight, selectedFlight }) => {
  ...
  // console.log(flight.id, selectedFlight?.id, selectedFlight === flight);
  ...
  return (
    <div className="flex flex-col w-full my-4 px-8 py-6 font-normal text-sm bg-slate-50 rounded-lg drop-shadow-md">
        ...
        <div className="flex justify-between items-center w-full py-1 mb-3">
          <div>
            ...
            <span className="text-gray-900 font-semibold capitalize">
              {flight.flight_class} class
            </span>
          </div>
          <span className="text-gray-900 font-semibold">{formatIDR.format(flight.price)}</span>
        </div>
      ...
      <div className="flex justify-end items-center w-full mt-5 ">
        <button className="uppercase px-6 py-2 ml-3 rounded-md font-semibold  text-gray-700 bg-none border border-gray-200 hover:bg-gray-200 ease-in-out duration-500">
          Wishlist
        </button>
        <button
          ...
          disabled={selectedFlight === flight}
          onClick={() => handleSelectFlight(flight)}
        >
          {selectedFlight === flight ? "Selected" : "Book Now"}
        </button>
      </div>
    </div>
  );
